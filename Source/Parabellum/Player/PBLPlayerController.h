 ...
 * Контроллер игрока.
 *
 * Ответственность: ввод и всё сугубо локальное — настройки мыши, HUD, меню.
 * Существует на сервере и на владеющем клиенте, но НЕ на чужих клиентах.
 *
 * Сюда НЕ кладём: состояние персонажа (оно в APBLCharacter, иначе не
 * реплицируется другим игрокам) и правила матча (они в APBLGameMode).
 */
UCLASS(Config=Game)
